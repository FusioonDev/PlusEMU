...
#pragma warning disable CS0414
        ...
        public void Run(object state)
        {
            try
            {
                ...
                {
                    // TODO: Make this code neat with less hard-coded values like respectes count
                    _player.GetStats().RespectsTimestamp = DateTime.Today.ToString("MM/dd");
                    using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        dbClient.RunQuery("UPDATE `user_stats` SET `dailyRespectPoints` = '" + (_player.Rank == 1 ? 10 : 20) + "', `dailyPetRespectPoints` = '" + (_player.Rank == 1 ? 10 : 20) + "', `respectsTimestamp` = '" + DateTime.Today.ToString("MM/dd") + "' WHERE `id` = '" + _player.Id + "' LIMIT 1");
                    }

                    _player.GetStats().DailyRespectPoints = _player.Rank == 1 ? 10 : 20;
                    _player.GetStats().DailyPetRespectPoints = _player.Rank == 1 ? 10 : 20;
