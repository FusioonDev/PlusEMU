...
namespace Plus.HabboHotel.Users
{
    public class Habbo
    {
        ...
        public Habbo(int id, string username, int rank, string motto, string look, string gender, int credits, int activityPoints, int homeRoom,
            ...
            int gotwPoints, bool ignoreInvites, double timeMuted, double tradingLock, bool allowGifts, int friendBarState, bool disableForcedEffects, bool allowMimic)
        {
            Id = id;
            Username = username;
            Rank = rank;
            ...
            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                try
                {
                    ...
                    if (Convert.ToString(statRow["respectsTimestamp"]) != DateTime.Today.ToString("MM/dd"))
                    {
                        ...
                        // TODO: Make respects not hard-coded
                        _habboStats.DailyRespectPoints = rank == 1 ? 10 : 20;
                        _habboStats.DailyPetRespectPoints = rank == 1 ? 10 : 20;

                        dbClient.RunQuery("UPDATE `user_stats` SET `dailyRespectPoints` = '" + _habboStats.DailyRespectPoints + "', `dailyPetRespectPoints` = '" + _habboStats.DailyPetRespectPoints + "', `respectsTimestamp` = '" + DateTime.Today.ToString("MM/dd") + "' WHERE `id` = '" + id + "' LIMIT 1");
                    }
                }
                catch (Exception e)
                {
